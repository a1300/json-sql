	...
	if (_.isObject(condition) && !_.isEmpty(condition)) {
		...
		if (_.isArray(condition)) {
			...
			result = _(condition).map(function (value, field) {
				...
				} else if (_.isObject(value) && BigNumber.isBigNumber(value)) {
					const operatorFn = self.dialect.conditions.get(params.operator);
					if (!operatorFn) {
						throw new Error('Unknown operator "' + operator + '"');
					}
					return operatorFn(field, params.operator, value.toString());
